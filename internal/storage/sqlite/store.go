...
func (s *Store) Load(ctx context.Context, tenantID string) (agent.StoredState, error) {
	var state agent.StoredState
...
func (s *Store) SaveThread(ctx context.Context, thread agent.StoredThread) error {
...
func (s *Store) SaveRun(ctx context.Context, run agent.StoredRun) error {
...
func (s *Store) AppendMessages(ctx context.Context, tenantID string, threadID string, messages []agent.HistoryMessage) error {
...
func (s *Store) OpenJournal(_ context.Context, scope agent.RunContext) (journaled.Journal, error) {
	return &journal{db: s.db, scope: scope}, nil
}

func (s *Store) ResetJournal(ctx context.Context, scope agent.RunContext) error {
	_, err := s.db.ExecContext(ctx,
...
func (s *Store) Find(ctx context.Context, scope task.Scope, position int, callHash string) (task.Record, bool, error) {
	record, err := s.scanTask(s.db.QueryRowContext(ctx, taskSelect+`
...
func (s *Store) Create(ctx context.Context, record task.Record) error {
	_, err := s.db.ExecContext(ctx, `
INSERT INTO tasks (
...
func (s *Store) Get(ctx context.Context, tenantID, taskID string) (task.Record, error) {
	record, err := s.scanTask(s.db.QueryRowContext(ctx, taskSelect+`WHERE tenant_id=? AND id=?`, tenantID, taskID))
	if errors.Is(err, sql.ErrNoRows) {
...
func (s *Store) List(ctx context.Context, tenantID, runID string) ([]task.Record, error) {
...
func (s *Store) Resolve(ctx context.Context, tenantID, taskID string, tokenHash []byte, resolution task.Resolution, now time.Time) (task.Record, error) {
	tx, err := s.db.BeginTx(ctx, nil)
	if err != nil {
...
func (s *Store) MarkExecuted(ctx context.Context, tenantID, taskID string, _ time.Time) error {
	result, err := s.db.ExecContext(ctx,
		...
		task.StateExecuted, tenantID, taskID)
	if err != nil {
		return err
	}
	affected, err := result.RowsAffected()
...
func (s *Store) loadThreads(ctx context.Context, tenantID string) ([]agent.StoredThread, error) {
...
FROM threads WHERE tenant_id=? ORDER BY created_at`, tenantID)
	...
	defer rows.Close()
	var out []agent.StoredThread
	for rows.Next() {
		var thread agent.StoredThread
...
func (s *Store) loadRuns(ctx context.Context, tenantID string) ([]agent.StoredRun, error) {
	rows, err := s.db.QueryContext(ctx, `
...
FROM runs WHERE tenant_id=? ORDER BY created_at`, tenantID)
	...
	defer rows.Close()
	var out []agent.StoredRun
	for rows.Next() {
		var run agent.StoredRun
		var created, updated string
...
func (s *Store) loadMessages(ctx context.Context, tenantID string) ([]agent.StoredMessage, error) {
	rows, err := s.db.QueryContext(ctx, `
...
FROM messages WHERE tenant_id=? ORDER BY thread_id,position`, tenantID)
	...
	defer rows.Close()
	var out []agent.StoredMessage
	for rows.Next() {
		message := agent.StoredMessage{TenantID: tenantID}
...
func scanTaskRows(row rowScanner) (task.Record, error) {
	return scanTaskRow(row)
}
